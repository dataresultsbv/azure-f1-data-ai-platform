...
class F1CLIENT:
    """
    Responsible for communication with the jolpica-f1-api
    with auto-pagination and rate-limiting resilience.
    """
    ...
    def _merge_payloads(self, base_payload: Dict[str, Any], new_payload: Dict[str, Any]) -> None:
        """
        Intelligently merges nested data lists,
        blending split pages for identical race rounds.
        """
        ...
        if "RaceTable" in base_mr and "RaceTable" in new_mr:
            ...
            for new_race in new_races:
                existing_race = next(
                    (r for r in base_races if r.get("round") == new_race.get("round")), None
                )
            ...
            if base_lists and new_lists:
                if "DriverStandings" in base_lists[0]:
                    ...
                    base_lists[0]["ConstructorStandings"].extend(
                        new_lists[0].get("ConstructorStandings", [])
                    )
...
class INGESTIONPIPELINE:
    ...
    def __init__(
        self, start_season: int, end_season: int, api_client: F1CLIENT, uploader: ADLSUPLOADER
    ):
    ...
    def run(self) -> None:
        ...
        for season in range(self.start_season, self.end_season + 1):
            ...
            try:
                ...
                blob_path = (
                    f"f1/constructor_standings/season={season}/"
                    f"bronze_constructor_standings_{season}.json"
                )
                self.uploader.upload_json(data, blob_path)
            except Exception as e:
...
def main():
    ...
    pipeline = INGESTIONPIPELINE(
        start_season=start_season, end_season=end_season, api_client=api_client, uploader=uploader
    )
