...
class F1Transformer:
    ...
    def extract_race_id_expr() -> pl.Expr:
        ...
        return (pl.col("season").cast(pl.Int32) * 100 + pl.col("round").cast(pl.Int32)).alias("raceId")

    def transform_results(self, json_bytes: bytes) -> Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame, pl.DataFrame, pl.DataFrame]:
        ...
        results_exploded = (
            ...
            .with_columns(
                pl.int_range(1, pl.len() + 1).over("raceId").alias("positionOrder")
            )
        )
        ...
        race_results = results_exploded.select([
            ...
            pl.col("status"),
            pl.col("FastestLap").struct.field("rank").cast(pl.Int32, strict=False).alias("fastest_lap_rank"),
            pl.col("FastestLap").struct.field("Time").struct.field("time").alias("fastest_lap_time"),
            pl.col("FastestLap").struct.field("AverageSpeed").struct.field("speed").cast(pl.Float64, strict=False).alias("fastest_lap_avgspeed")
        ])
    ...
    def transform_driver_standings(self, json_bytes: bytes) -> pl.DataFrame:
        ...
        standings_exploded = (
            raw_df
            .select(pl.col("MRData").struct.field("StandingsTable").struct.field("StandingsLists"))
...
class F1Orchestrator:
    ...
    def __init__(self, seasons: List[int], storage_handler: F1StorageHandler, transformer: F1Transformer):
    ...
    def run(self) -> None:
        ...
        for season in self.seasons:
            ...
            try:
                ...
                self.storage.write_silver_partitioned(constructor_standings, "constructor_standings", season)
            except Exception as e:
                logger.error(f"ERROR - Failed processing constructor standings for season {season}: {e}")
...
if __name__ == "__main__":
    ...
    pipeline = F1Orchestrator(seasons=seasons_to_process, storage_handler=handler, transformer=transformer)
    pipeline.run()