...
from f1_to_silver import F1Transformer
...
def test_transform_results_parses_nested_structures(mock_bronze_results_json):
    ...
    circuits, races, drivers, constructors, race_results = transformer.transform_results(
        mock_bronze_results_json
    )
    ...
    assert races["raceId"][0] == expected_race_id
    assert race_results["raceId"][0] == expected_race_id
