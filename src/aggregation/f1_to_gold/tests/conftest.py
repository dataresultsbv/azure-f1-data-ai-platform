...
def mock_config(monkeypatch):
    monkeypatch.setenv("AZURE_STORAGE_ACCOUNT_URL", "https://myaccount.dfs.core.windows.net")

    monkeypatch.setenv("BLOB_CONTAINER_NAME", "silver")
...
def engine(mock_config, mock_client):
    return GoldAggregationEngine(config=mock_config, client=mock_client)
...
@pytest.fixture
def parquet_test_data(tmp_path, race_results_df, driver_standings_df, drivers_df):
    ...
    driver_standings_df.write_parquet(driver_standings_path)

    drivers_df.write_parquet(drivers_path)
