...
resource "azurerm_container_group" "f1_container_group" {
  name                = "ci-${var.container_image.key}-${var.resource_name_suffix}"
  location            = var.location
  resource_group_name = var.resource_group_name
  os_type             = "Linux"
  ...
  container {
    ...
    image  = "${azurerm_container_registry.container_registry.name}.azurecr.io/${var.container_image.key}:latest"
    cpu    = "0.5"
    memory = "1.0"
